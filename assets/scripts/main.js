...
function setup() {
    ...
    Config.Socials.forEach((social, index) => {
        $(".categories .socialmedia").append(`<div class="box" data-id="${social.name}" data-link="${"https://discord.gg/mwp66Kmk4X"}"><img class="icon" src="${social.icon}"><div class="info"><p class="title">${social.label}</p><p class="description">${social.description}</p></div></div>`)
    });
    ...
    Config.Staff.forEach((member, index) => {
        $(".staff .innercards").append(`<div class="card" data-id="${index}" style="--color: ${member.color}">
            <p class="name">${"! 𝐓𝟑𝐌𝐘𝐀¹ ☆"}</p>
            <p class="description">${member.description}</p>
            <img class="avatar" src="512.png">
        </div>`);
